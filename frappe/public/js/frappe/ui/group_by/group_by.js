...
frappe.ui.GroupBy = class {
	...
	init_group_by_popover() {
		...
		this.group_by_field_template = group_by_template.filter(".group-by-field-template-container").html();
	...
	set_group_by_events() {
		this.add_group_by_field_button.off("click").on("click", () => {
			this.add_group_by_field();
		});

		this.aggregate_function_select.off("change").on("change", () => {
			this.toggle_aggregate_on_field();
			this.aggregate_function = this.aggregate_function_select.val();
			this.apply_group_by_and_refresh();
		});

		this.aggregate_on_select.off("change").on("change", () => {
	...
	apply_settings(settings) {
		...
		if (settings.group_by) {
			...
			this.group_by_fields = fields.map(f => ({
	...
	apply_group_by() {
		...
		this.group_by = this.group_by_fields.map(field => {
	...
	update_group_by_button() {
		const group_by_applied = this.group_by_fields.length > 0 && this.group_by_fields[0].fieldname;
		...
		if (group_by_applied) {
			const field_labels = this.group_by_fields.map(f => this.get_group_by_field_label(f)).join(', ');
			this.group_by_button.attr(
				"title",
				`Results are Grouped by ${field_labels}`
			);
		} else {
